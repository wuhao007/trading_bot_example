...
def _GetMarkets(vs_currency):
  coins_list = requests.get(f'https://api.coingecko.com/api/v3/coins/markets?vs_currency={vs_currency}&order=market_cap_desc').json() 
  return [coin['id'] for coin in coins_list]


def _GetMarketChart(coin, vs_currency):
  market_chart = requests.get(f'https://api.coingecko.com/api/v3/coins/{coin}/market_chart?vs_currency={vs_currency}&days=36500&interval=hourly').json()
  return [price for price in market_chart['prices'] if price[1]]


def _GetPrice(coin, vs_currency):
  price = requests.get(f'https://api.coingecko.com/api/v3/simple/price?ids={coin}&vs_currencies={vs_currency}').json()
...
def _Date2Timestamp(item):
  return datetime.datetime.timestamp(datetime.datetime.strptime(item, '%Y-%m-%dT%H:%M:%S')) * 1000
...
def _GetAhr999(items, w, b, start_date):
  end_item = items[-1]
  return end_item[1] ** 2 / (_GetAvg(items) * _GetLogPrice(end_item[0], w, b, start_date))


def _GetAhr999x(items, w, b, start_date):
  end_item = items[-1]
  return _GetAvg(items) * _GetLogPrice(end_item[0], w, b, start_date) * 3 / (end_item[1] ** 2)


def _GetAhr999Prices(coin, vs_currency):
  ahr999_prices = _GetMarketChart(coin, vs_currency)[-_AHR999_DAYS-1:][:_AHR999_DAYS]
...
def _GetPastAhr999(coin, w, b, start_date, vs_currency):
  ...
  print(f'Log price: {_GetLogPrice(ahr999_prices[-1][0], w, b, start_date)} {vs_currency}')
...
def _GetMarketCap(coin):
  ...
  while page == 1 or markets:
    try:
      markets = requests.get(f'https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&order=market_cap_desc&per_page=250&page={page}&sparkline=false').json()
    except:
      time.sleep(sleep)
      sleep *= 2
      continue
...
def _GetJzr(coin):
  ...
  print(f'{sum((jzr_prices[i][1]/jzr_prices[i-1][1] - 1) for i in range(1, len(jzr_prices))) * 100}%')
...
def _GetStartDate(market_chart):
  return (_ParseDate(market_chart[0][0]) - datetime.timedelta(days=1)).isoformat()
...
def _GetHaowu999(coin, vs_currency, start_date=None):
  ...
  _GetTodayAhr999(coin, w, b, start_date, vs_currency)
